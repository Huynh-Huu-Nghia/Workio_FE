...
import { Mail, Phone, MapPin, Briefcase, Calendar as CalendarIcon, Edit3, Loader2 } from "lucide-react";
import { Link } from "react-router-dom";
import path from "@/constants/path";
...
export default function CandidateHome() {
  ...
  const fullAddress = useMemo(() => {
    if (!addrInfo.province_code && !addrInfo.street) return "Chưa cập nhật địa chỉ";

    const pName = provinces?.find((p: any) => p.code == addrInfo.province_code)?.name;
  ...
  const tileContent = ({ date, view }: any) => {
    if (view === 'month' && stats.interviewStats.events.length > 0) {
      const hasEvent = stats.interviewStats.events.some((e: any) => 
        new Date(e.date).toDateString() === date.toDateString()
      );
      if (hasEvent) return <div className="h-1.5 w-1.5 bg-red-500 rounded-full mx-auto mt-1"></div>;
    }
    return null;
  };

  const selectedDateEvent = stats.interviewStats.events.find((e: any) => 
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col items-center text-center h-full">
            <div className="relative">
              ...
              <Link to={path.CANDIDATE_PROFILE} className="absolute bottom-4 right-0 bg-white p-1.5 rounded-full shadow border border-gray-200 text-gray-500 hover:text-orange-500">
                <Edit3 size={14} />
              </Link>
            </div>
            ...
            <div className="w-full space-y-4 text-left bg-gray-50 p-4 rounded-lg">
              <div className="flex items-center gap-3 text-gray-700">
                ...
                <span className="truncate text-sm" title={profile.email}>{profile.email}</span>
              </div>
              
              <div className="flex items-center gap-3 text-gray-700">
                <Phone size={18} className="text-orange-500 flex-shrink-0" /> 
                <span className="text-sm">
                    {profile.phone || <span className="text-gray-400 italic">Chưa cập nhật SĐT</span>}
                </span>
              </div>
            ...
            <div className="mt-auto w-full pt-6">
               <Link to={path.CANDIDATE_PROFILE} className="block w-full py-2.5 bg-orange-600 text-white rounded-lg font-medium hover:bg-orange-700 transition-colors shadow-orange-200 shadow-lg text-center">
                 Cập nhật hồ sơ
               </Link>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col items-center relative min-h-[300px]">
               <h3 className="font-bold text-gray-700 flex items-center gap-2 w-full mb-2">
                  <Briefcase size={20} className="text-blue-500"/> Việc làm đã ứng tuyển
               </h3>
                 ...
                 <>
                   <div className="h-48 w-full relative">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie data={stats.jobStats.data} innerRadius={55} outerRadius={75} paddingAngle={5} dataKey="value">
                            {stats.jobStats.data.map((entry: any, index: number) => (
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                        <span className="text-3xl font-extrabold text-gray-800">{stats.jobStats.total}</span>
                   ...
                   <div className="flex flex-wrap justify-center gap-3 text-xs mt-4">
                      {stats.jobStats.data.map((item: any) => (
                         <div key={item.name} className="flex items-center gap-1">
                            ...
                            <span className="text-gray-600">{item.name}: <b>{item.value}</b></span>
                         </div>
                      ))}
                   </div>
                 </>
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-gray-400">
                     ...
                     <Link to={path.CANDIDATE_JOBS} className="text-orange-500 text-sm hover:underline mt-1">Tìm việc ngay</Link>
                  </div>
               )}
            </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col items-center relative min-h-[300px]">
               <h3 className="font-bold text-gray-700 flex items-center gap-2 w-full mb-2">
                  <CalendarIcon size={20} className="text-purple-500"/> Lịch phỏng vấn
               </h3>
               {stats.interviewStats.total > 0 ? (
                 <>
                   <div className="h-48 w-full relative">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie data={stats.interviewStats.chartData} innerRadius={55} outerRadius={75} paddingAngle={5} dataKey="value">
                            {stats.interviewStats.chartData.map((entry: any, index: number) => (
                              <Cell key={`cell-${index}`} fill={entry.color} />
                            ))}
                          </Pie>
                          <RechartsTooltip />
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                        <span className="text-3xl font-extrabold text-gray-800">{stats.interviewStats.total}</span>
               ...
               <div className="md:w-1/3 bg-blue-50 rounded-xl p-4 flex flex-col">
                  ...
                  {selectedDateEvent ? (
                    <div className="animate-fadeIn">
                      <p className="font-bold text-gray-800 mb-1">{selectedDateEvent.title}</p>
                      <div className="text-xs text-gray-600 space-y-1">
                        <p>🕒 {new Date(selectedDateEvent.date).toLocaleTimeString('vi-VN', {hour: '2-digit', minute:'2-digit'})}</p>
                        <p>📍 {selectedDateEvent.location}</p>
                        {selectedDateEvent.status && <p>🏷️ {selectedDateEvent.status}</p>}
                      </div>
                    </div>
                  ) : (
