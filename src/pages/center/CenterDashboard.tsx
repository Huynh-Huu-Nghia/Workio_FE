...
const CenterDashboard: React.FC = () => {
  const { user } = useUser();
  const { data: statsResponse, isLoading, isError } = useCenterStatisticsQuery();
    ...
    return [
      {
        ...
        descriptor: stats.courses.upcoming > 0 
      ...
      },
      {
        ...
        trend: stats.learners.byStatus.pending > 0 
      ...
      },
      {
        ...
        trendTone: stats.completionRate >= 80 ? "text-blue-700 bg-blue-100" : "text-amber-700 bg-amber-100",
  ...
  const statusBreakdown = useMemo(() => {
    ...
    return [
      { 
        ...
        percent: total > 0 ? Math.round((stats.learners.byStatus.learning / total) * 100) : 0, 
        bar: "bg-blue-500" 
      },
      { 
        id: "pending", 
        ...
        percent: total > 0 ? Math.round((stats.learners.byStatus.pending / total) * 100) : 0, 
        bar: "bg-amber-500" 
      },
      { 
        id: "completed", 
        ...
        percent: total > 0 ? Math.round((stats.learners.byStatus.completed / total) * 100) : 0, 
        bar: "bg-emerald-500" 
      },
      { 
        id: "rejected", 
        ...
        percent: total > 0 ? Math.round((stats.learners.byStatus.rejected / total) * 100) : 0, 
        bar: "bg-rose-500" 
      },
    ];
  }, [stats]);
      ...
      <CenterLayout title="Báo cáo đào tạo">
        <div className="flex min-h-[400px] items-center justify-center">
          <div className="text-center">
            ...
            <h3 className="mt-4 text-lg font-semibold text-gray-900">Không thể tải dữ liệu</h3>
      ...
      <div className="space-y-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-6">
            <div>
              ...
              <p className="mt-2 text-base text-white/80">
                Xin chào <span className="font-semibold text-white">{displayName}</span>, đây là tổng quan hiệu suất các khóa học.
              </p>
            </div>
        ...
        <section className="grid gap-6 lg:grid-cols-3">
          <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm lg:col-span-2">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500">Tổng {stats.learners.total} học viên</p>
              </div>
              ...
              {statusBreakdown.map((status) => (
                <div key={status.id}>
                  <div className="flex items-center justify-between text-sm font-semibold text-gray-700">
                    ...
                    <span>{status.count} học viên ({status.percent}%)</span>
                  </div>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <div className="rounded-2xl bg-indigo-50 p-4">
                <p className="text-xs font-semibold uppercase text-indigo-500">Thời lượng TB</p>
                <p className="mt-2 text-2xl font-bold text-indigo-900">{stats.duration.avgHours}h</p>
                <p className="text-xs text-indigo-500">{stats.duration.coursesWithDuration} khóa có thời lượng</p>
              </div>
              <div className="rounded-2xl bg-emerald-50 p-4">
                <p className="text-xs font-semibold uppercase text-emerald-500">Tổng thời lượng</p>
                <p className="mt-2 text-2xl font-bold text-emerald-900">{stats.duration.totalHours}h</p>
          ...
          <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-900">Xu hướng tăng trưởng</h3>
                <p className="text-sm text-gray-500">Khóa học được tạo</p>
              </div>
            </div>
            <div className="mt-6 space-y-4">
              <div className="rounded-2xl border border-gray-100 px-4 py-3">
                <p className="text-xs font-semibold text-gray-500">3 tháng gần đây</p>
                <div className="mt-1 flex items-end justify-between">
                  <p className="text-2xl font-bold text-gray-900">{stats.trends.coursesLast3Months}</p>
                  <span className="text-xs font-semibold text-blue-600">khóa học</span>
                </div>
              </div>
              <div className="rounded-2xl border border-gray-100 px-4 py-3">
                <p className="text-xs font-semibold text-gray-500">Tháng này</p>
                <div className="mt-1 flex items-end justify-between">
                  <p className="text-2xl font-bold text-gray-900">{stats.trends.coursesThisMonth}</p>
                  <span className="text-xs font-semibold text-green-600">khóa học</span>
                </div>
              </div>
              <div className="rounded-2xl bg-gradient-to-br from-purple-50 to-purple-100 px-4 py-3">
                <p className="text-xs font-semibold text-purple-700">Tổng khóa</p>
                <p className="mt-1 text-3xl font-bold text-purple-900">{stats.courses.total}</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold text-gray-900">Top khóa học theo học viên</h2>
                <p className="text-sm text-gray-500">Xếp hạng theo số lượng đăng ký</p>
              </div>
              <Link to={`${path.CENTER_COURSES}#manage`} className="text-sm font-semibold text-indigo-600 hover:text-indigo-800">
                Xem tất cả
              </Link>
            </div>
            {stats.topCourses.byLearners.length > 0 ? (
              <div className="mt-6 divide-y divide-gray-100">
                {stats.topCourses.byLearners.map((course, index) => (
                  <div key={course.course_id} className="flex items-center justify-between gap-4 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{course.learners} học viên</p>
                      </div>
                    </div>
              ...
              <div className="mt-6 flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 p-8 text-center">
                ...
                <p className="mt-2 text-sm font-medium text-gray-600">Chưa có dữ liệu</p>
              </div>
            )}
          </div>
          ...
          <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-900">Thống kê theo lĩnh vực</h3>
                <p className="text-sm text-gray-500">Phân bố khóa học theo training field</p>
              </div>
            </div>
            {stats.byTrainingField.length > 0 ? (
              <div className="mt-6 space-y-3">
                {stats.byTrainingField.slice(0, 5).map((field) => (
                  <div key={field.field} className="rounded-2xl border border-gray-100 p-4">
                    <div className="flex items-center justify-between">
                      <div>
              ...
              <div className="mt-6 flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 p-8 text-center">
                ...
                <p className="mt-2 text-sm font-medium text-gray-600">Chưa có dữ liệu lĩnh vực</p>
              </div>
            )}
          </div>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-900">Tỷ lệ hoàn thành</h3>
                <p className="text-sm text-gray-500">Tổng quan về hiệu suất khóa học</p>
              </div>
            </div>
            <div className="mt-6 flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex-1">
                ...
                <p className="mt-2 text-xs text-gray-500">
                  {stats.learners.byStatus.completed} / {stats.learners.total} học viên hoàn thành
                </p>
              </div>
              <div
              ...
              >
                ...
                >
                  {stats.completionRate >= 80 ? 'Xuất sắc' : stats.completionRate >= 60 ? 'Tốt' : 'Cần cải thiện'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-gray-900">Top khóa học theo tỷ lệ hoàn thành</h3>
                  <p className="text-sm text-gray-500">Khóa học có hiệu suất cao nhất</p>
                </div>
              </div>
            </div>
            {stats.topCourses.byCompletion.length > 0 ? (
              <div className="mt-6 divide-y divide-gray-100">
                {stats.topCourses.byCompletion.map((course, index) => (
                  <div key={course.course_id} className="flex items-center justify-between gap-4 py-3">
                    <div className="flex items-center gap-3">
                    ...
                    <div className="text-right">
                      <p className="text-lg font-bold text-emerald-600">{course.completionRate}%</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-6 flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 p-8 text-center">
                ...
                <p className="mt-2 text-sm font-medium text-gray-600">Chưa có dữ liệu</p>
                <p className="text-xs text-gray-500">Các khóa học cần tối thiểu 5 học viên</p>
              </div>
            )}
          </div>
        </section>        </div>      </div>
